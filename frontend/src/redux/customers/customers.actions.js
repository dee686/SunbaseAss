 ...
 * */
export const getAllCustomersAction = (toastMsg,pageNo,searchKey, searchValue) => async (dispatch) => {
     if (!toastMsg) return;

     dispatch({ type: customersTypes.CUSTOMER_LOADING });
     let url;
// check search option 
     if(searchKey != null && searchValue != null) url = `http://localhost:8080/all?page=${pageNo}&size=7&${searchKey}=${searchValue}&sortBy=id&sortDir=asc`;
     else url = `http://localhost:8080/all?page=${pageNo}&size=7&sortBy=id&sortDir=asc`;
     console.log(url);
     try {
...
export const udpateCustomerAction = (toastMsg, customerId, update) => async (dispatch) => {
     ...
     try {
          ...
          if (res.ok) {
               dispatch(getAllCustomersAction(toastMsg,0,null,null))
